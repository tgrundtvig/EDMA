...
public class ValueTokenizer
{
	...
	public ValueToken getNextToken()
	{
		...
		if(curPos == s.length()) return new ValueToken(ValueToken.TokenType.EOF, null);
		char c = s.charAt(curPos);
		switch(c)
		{
			case BEGIN:
				++curPos;
				return new ValueToken(ValueToken.TokenType.Begin, null);
			case END:
				++curPos;
				return new ValueToken(ValueToken.TokenType.End, null);
			case SEPERATOR:
				++curPos;
				return new ValueToken(ValueToken.TokenType.Seperator, null);
			case '"':
				return new ValueToken(ValueToken.TokenType.String, parseString());
			default: 
				return new ValueToken(ValueToken.TokenType.Value, parseValue());
		}
	}
