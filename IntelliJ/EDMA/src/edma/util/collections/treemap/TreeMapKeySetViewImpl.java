...
import edma.util.collections.intf.NavMap.MapCursor;
import edma.util.collections.intf.NavSet;
...
public class TreeMapKeySetViewImpl<K, V> implements NavSet<K>
{
	...
	{
		MapCursor<K,V> c = asOrderedMapCursor(cursor);
	...
	private MapCursor<K,V> asOrderedMapCursor(SetCursor<K> cursor)
	{
		if(cursor.getOrderedSet() != this) throw new IllegalArgumentException("The cursor is not owned by this set!");
		return (MapCursor<K,V>) ((SetCursorImpl) cursor).cursor;
	}
	
	private class SetCursorImpl implements SetCursor<K>
	{
		protected MapCursor<K,?> cursor;
		
		public SetCursorImpl(MapCursor<K,?> cursor)
		{
			this.cursor = cursor;
		}
